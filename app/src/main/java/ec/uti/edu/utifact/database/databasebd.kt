...
public class databasebd (context: Context): SQLiteOpenHelper(
    ...
    fun getProductos(): List<Producto> {
        ...
        if (cursor.moveToFirst()) {
            do {
                ...
                //productos.add(Producto( nameProd, provedProd, stock, precio))
            } while (cursor.moveToNext())
        }
    ...
    fun getUsuariosSearch(usuario: String): List<User> {
        val usuarios = mutableListOf<User>()
        val db = this.readableDatabase
        val cursor = db.rawQuery("SELECT * FROM usuarios where user =?", arrayOf(usuario))
    ...
    fun getFacturas(): List<Factura> {
        ...
        if (cursor.moveToFirst()) {
            println("Número de registros en la tabla factura: ${cursor.count}")
            do {
                val id = cursor.getInt(0)
    ...
    fun updateUsuario(id: Int, nombre: String, password: String, rol: Int): Int {
        val db = writableDatabase
        val values = ContentValues().apply {
            put(COLUMN_USER_NAME, nombre)
            put(COLUMN_USER_PASSWORD, password)
            put(COLUMN_USER_ROLE, rol)
        }
        return db.update(TABLE_USERS, values, "id = ?", arrayOf(id.toString()))
    }
}
